...
- 사용자가 요청한 URL 링크를 요청했을 때, 어떤 특정한 페이지로 연결 할 것인지 결정하는 것

### SPA

- 하나의 URL로 한번 페이지가 로딩되고 나면 다른 새로운 페이지가 열리는 것이 아니라 부분적으로 업데이트되도록 작동
- 리액트는 SPA을 쉽게 만들 수 있도록 도와주는 라이브러리
- SPA 단점: 특정 페이지 북마크가 안되고, 뒤로가기 앞으로가기와 같은 네비게이션 사용이 되지 않음
- 위의 단점을 보완하기 위해 React Router를 사용

### React Router

- SPA의 장점인 부분 업데이트를 유지하면서 뒤로가기,앞으로가기 등 네비게이션 기능과 북마크 기능도 사용 가능하도록 개발 가능
